...
public class NewPieChartPanel extends RequirementPanel{
	...
	/**
	 * @param title the title of the pie chart which determines if we are displaying based on status, iteration, or users assigned to.
	 */
	...
	/**
	 * @return the data with the percentage of requirements with a given status to be displayed by the pie chart
	 */
		...
		for(int i = 0; i < iterations.size(); i++){
			for(int j = 0; j < requirements.size(); j++){
				if(requirements.get(j).getIteration().toString().equals(iterations.get(i).toString())){
					iterationNum[i]++;//increments the number if the requiremet belongs to the given iteration
				}
			}
		}
		for(int k = 0; k < iterationNum.length; k++){
			dataSet.setValue(iterations.get(k).toString(), iterationNum[k]);//sets the data
		}
		return dataSet;
	}
	...
	/**
	 * @return the data of the number of requirements a user has assigned to them
	 */
	private static PieDataset setDataAssignTo(){
		...
		List<Requirement> requirements = RequirementModel.getInstance().getRequirements();//list of requirements
		for(int i = 0; i < requirements.size(); i++){
			List<String> users = requirements.get(i).getAssignedTo();//list of users for specific requirement
		...
		for(int i = 0; i < requirements.size(); i++){
			...
			for(int j = 0; j < userNames.size(); j++){
				if(users.contains(userNames.get(j))){
					numReqAssigned[j]++;//count the number of requirements a user has assigned to them
				}
			}
		}
		for(int k = 0; k < userNames.size(); k++){
			dataSet.setValue(userNames.get(k), numReqAssigned[k]);//populate pie chart data
		}
		return dataSet;
	}
/**
 * @param dataset the data to be displayed by the pie chart
...
private static JFreeChart createChart(PieDataset dataset, String title) {
        ...
        PiePlot3D plot = (PiePlot3D) chart.getPlot();//3D pie chart. the cats are going to love this.
