...
public abstract class Authenticator {
    ...
    public Authenticator(String type) {
        authType = type;
        passwordHash = new Sha256Password();
    }
    ...
    public Session login(String postString) throws AuthenticationException, WPISuiteException {
        ...
        try {
            ...
            logger.log(Level.WARNING, "Login attempted with non-existant user <" + credentials[0]
                    + ">");
            throw new AuthenticationException("The user \"" + credentials[0]
                    + "\" could not be found. Please check if the username was spelled correctly.");
        }
        ...
        String hashedPassword = passwordHash.generateHash(credentials[1]);
        if (!user.matchPassword(hashedPassword)) {
            logger.log(Level.WARNING, "Login attempted with bad password for User <"
                    + credentials[0] + ">");
            throw new AuthenticationException(
                    "An invalid password was given. Please check the password and try again.");
        }
